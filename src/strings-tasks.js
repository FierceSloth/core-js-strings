...
function isString(value) {
  let result = value;
  if (value instanceof String) result = value.valueOf();
...
function sumOfCodes(str) {
  ...
  const result = strArr.reduce((sum, current) => sum + current.charCodeAt(), 0);
  return result;
}
...
function orderAlphabetically(str) {
  const sortedStr = str.split('').sort((a, b) => a.localeCompare(b));
...
function isPalindrome(str) {
  ...
  return cleanString.toLowerCase() === reversedStr.toLowerCase();
}
...
function reverseWords(str) {
  const strArr = str.split(' ');
  const reversedWords = strArr.map((i) => i.split('').reverse().join(''));
...
function invertCase(str) {
  const strArr = str.split('');
  const invertWords = strArr.map((i) => {
    return i === i.toLowerCase() ? i.toUpperCase() : i.toLowerCase();
  });
  return invertWords.join('');
}
...
function extractNameFromTemplate(value) {
  const cleanString = value.replace(/Hello, /g, '').replace(/!/g, '');
  return cleanString;
}
...
function encodeToRot13(str) {
  const rot13Array =
    '0ANBOCPDQERFSGTHUIVJWKXLYMZanbocpdqerfsgthuivjwkxlymz'.split('');
  const strArr = str.split('');

  const resultArr = strArr.map((item) => {
    const rotIndex = rot13Array.indexOf(item);
    if (rotIndex !== -1) {
      return rotIndex % 2 === 0
        ? rot13Array[rotIndex - 1]
        : rot13Array[rotIndex + 1];
    }
    return item;
  });
...
function getCardId(value) {
  ...
  return CARD_DESK.indexOf(value);
}
